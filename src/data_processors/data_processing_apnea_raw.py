...
class DataProcessorRSWA:
    ...
    def __init__(self, input_path = '/Volumes/TOSHIBA EXT/training_data',
                 ...
                 channel_list = ['Abdomen','Chest','Airflow','SpO2','P-Flo','ECG'],
                 ...
                 ecg_filter_lower = True, ecg_lower_bound = 0.3, 
...
class DataProcessorApneaMP(DataProcessorApnea):
    ...
    def _process_helper(self, f):
        try:
            ...
            stages = stages[0:epochs.index(final_epoch)+1]
            stage_dict = {}
            stage_dict[ID] = {epoch: stages[i] for i, epoch in enumerate(epochs)}
            #self.stage_dict[ID] = stage_dict
            
            # Prepare for for making apnea channels
            X = np.zeros((end_time*self.fs,self.n_channels))
            print(f'Shape of X: {X.shape}')
            apnea_hypoapnea_targets = {}
            apnea_hypoapnea_targets[ID] = convert_to_sequence(time_list = self._extract_apnea_hypopnea_events(xdf),
                                                                             end_time = end_time, sampling_rate = self.fs)
                ...
                if c in ['Abdomen','Chest','P-Flo','Airflow']:
                    ...
                    if self.hr == True:
                        ...
                        try:
                            # Sometimes too few heartbeats to calculate rate
                            ecg_sig = sig.ravel()
                            ecg_data = hamilton_segmenter(signal = ecg_sig, sampling_rate = ecg_sampling_rate)
                            mf = MedianFilter(filter_lower = self.ecg_filter_lower, filter_upper = self.ecg_filter_upper,
                                              upper_bound = self.ecg_upper_bound, lower_bound = self.ecg_lower_bound, 
                                              output_sampling_rate = self.fs)
                            hr_ts = mf.generate_hr_time_series(ecg_data = ecg_data, ecg_sig = ecg_sig, end_time = end_time)
                            X[:,i] = hr_ts[0:self.fs*end_time]
                        except Exception as e:
                            print("Error in calculating HR!!", e)
                            continue
            ...
            file_name = f"{ID}"
            np.save(self.output_data_path.joinpath(file_name),X)
            return (stage_dict,apnea_hypoapnea_targets)
    ...
    def process_data(self):
        if self.ID_list is not None:
            self.files = list(filter(lambda x, l = self.ID_list: any([x.stem in f for f in l]), self.files))
        
        with mp.Pool(processes=4) as pool:
                results = list(pool.map(self._process_helper, self.files))
        
        for (staging, apneas) in results:
            self.stage_dict.update(staging)
            self.apnea_hypoapnea_targets.update(apneas)
