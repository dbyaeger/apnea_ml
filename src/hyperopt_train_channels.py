...
class HyperOptimizer():
    ...
    def run_with(self, Abdomen, Airflow, Chest, ECG, P_Flo, SpO2, normalizer):
        ...
        """
        select_channels = []
        if Abdomen: select_channels.append('Abdomen')
        if Airflow: select_channels.append('Airflow')
        if Chest: select_channels.append('Chest')
        if ECG: select_channels.append('ECG')
